...
model ThreeElements "Thermal Zone with three elements for exterior walls,
  ...
  // 23 Nov --- Index selector for thermSplitter ports for floor ---
  ...
  parameter Integer nFloor(min = 1) "Number of RC-elements of floor plate"
    annotation(Dialog(group="Floor plate"));
  parameter Modelica.Units.SI.ThermalResistance RFloor[nExt](each min=Modelica.Constants.small)
  ...
  parameter Modelica.Units.SI.ThermalResistance RFloorRem(min=Modelica.Constants.small)
    ...
    annotation (Dialog(group="Floor plate"));
  parameter Modelica.Units.SI.HeatCapacity CFloor[nExt](each min=Modelica.Constants.small)
...
protected
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resExtWallFloor(
    final G=min(ATotExt, AFloor)*hRad) if  AFloor > 0
    "Resistor between exterior walls and floor"
    annotation (                                      // 23 Nov if  ATotExt > 0 and AFloor > 0
                Placement(
    transformation(
    extent={{-10,-10},{10,10}},
    rotation=-90,
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resIntWallFloor(
    final G=min(AFloor, AInt)*hRad) if AFloor > 0
    "Resistor between interior walls and floor"
    annotation (                                  // 23 Nov if  AInt > 0 and AFloor > 0
                Placement(
    transformation(
    extent={{-10,-10},{10,10}},
    origin={204,-106})));
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resFloorWin(
   final G=min(ATotWin, AFloor)*hRad) if AFloor > 0
    "Resistor between floor plate and windows"
    annotation (                                    // 23 Nov if  ATotWin > 0 and AFloor > 0
                Placement(
    transformation(
    extent={{-10,-10},{10,10}},
    rotation=-90,
  ...
  parameter Integer dimension =  (if ATotExt > 0 then 1 else 0)+ (if ATotWin > 0 then 1 else 0)+ (if AInt    > 0 then 1 else 0)+ (if AFloor  > 0 then 1 else 0)
...
equation
  ...
  // --- Unconditional, legal connections ---
  connect(thermSplitterIntGains.portOut[idxFloorGains], floorRC.port_a);
  connect(floorRC.port_a, thermSplitterSolRad.portOut[idxFloorGains]);
/*
...
*/
  ...
  <p align=\"center\">
  <img src=\"modelica://IBPSA/Resources/Images/ThermalZones/ReducedOrder/RC/ThreeElements.png\" alt=\"image\"/>
  </p>
  </html>"));
end ThreeElements;
