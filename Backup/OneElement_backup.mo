...
model OneElement_backup "Thermal Zone with one element for exterior walls"
  ...
  parameter Modelica.Units.SI.CoefficientOfHeatTransfer hRad
    ...
    annotation (Dialog(group="Thermal zone"));
  final parameter Integer nOrientations(min=1) "Number of orientations"
    annotation(Dialog(group="Thermal zone"));
  parameter Integer nPorts=0 "Number of fluid ports"
    annotation(Evaluate=true,
    Dialog(connectorSizing=true, tab="General",group="Ports"));
  parameter Modelica.Units.SI.Area AWin[nOri]
    "Vector of areas of windows by orientations"
    annotation (Dialog(group="Windows"));
  parameter Modelica.Units.SI.Area ATransparent[nOri] "Vector of areas of transparent (solar radiation transmittend) elements by
  ...
  parameter Modelica.Units.SI.Area AExt[nOri]
...
// 24 Nov
  final parameter Boolean hasExt(fixed=true) =(sum(AExt) > 0) annotation(Evaluate=true);

  final parameter Boolean hasWin(fixed=true) =  (sum(AWin) > 0) annotation(Evaluate=true);

  final parameter Boolean hasSolRad =(sum(ATransparent) > 0) annotation(Evaluate=true);

  final parameter Boolean hasAir = (VAir > 0) annotation(Evaluate=true);

  final parameter Integer nOri =  nOrientations annotation(Evaluate=true);

  final parameter Integer nDim =(if hasExt then 1 else 0)+ (if hasWin then 1 else 0)
    annotation(Evaluate=true);



  Modelica.Blocks.Interfaces.RealInput solRad[nOri](
    each final quantity="RadiantEnergyFluenceRate",
    each final unit="W/m2") if hasSolRad
  ...
  Modelica.Blocks.Interfaces.RealOutput TAir(
    ...
    final unit="K",
    displayUnit="degC") if hasAir and not use_moisture_balance
    "Indoor air temperature"
    annotation (                    // 23 Nov if ATot > 0 or VAir > 0
                Placement(transformation(extent={{240,150},{260,170}}),
    iconTransformation(extent={{240,150},{260,170}})));

  Modelica.Blocks.Interfaces.RealOutput TRad(
    final quantity="ThermodynamicTemperature",
    final unit="K",
    displayUnit="degC"  // 23 Nov if ATot > 0
  ...
  Modelica.Fluid.Vessels.BaseClasses.VesselFluidPorts_b ports[nPorts](
    ...
    origin={150,-179.5})));
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a extWall
    "Ambient port for exterior walls"
    annotation (                                              // 23 Nov if ATotExt > 0
                Placement(transformation(
    extent={{-250,-50},{-230,-30}}), iconTransformation(extent={{-250,-50},{
            -230,-30}})));
  Modelica.Thermal.HeatTransfer.Interfaces.HeatPort_a window
    "Ambient port for windows"
    annotation (                                             // 23 Nov if ATotWin > 0
                Placement(transformation(extent={{-250,30},{-230,50}}),
 ...
 if indoorPortExtWalls "Auxiliary port at indoor surface of exterior walls"
  ...
  Fluid.MixingVolumes.MixingVolume volAir(
    ...
    final use_C_flow=use_C_flow) if hasAir and not use_moisture_balance and not use_moisture_balance
    "Indoor air volume"
  ...
  Fluid.MixingVolumes.MixingVolumeMoistAir volMoiAir(
    ...
    final C_nominal=C_nominal,
    final mSenFac=mSenFac,
    final use_C_flow=use_C_flow) if hasAir and not use_moisture_balance and use_moisture_balance
    "Indoor air volume"
    annotation (Placement(transformation(extent={{-20,-26},{0,-6}})));
  Modelica.Thermal.HeatTransfer.Components.ThermalResistor resWin(final R=RWin) "Resistor for windows"
    annotation (
    // 23 Nov if ATotWin > 0  "Resistor for windows"
                Placement(transformation(extent={{-180,30},{-160,50}})));
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow convHeatSol(
    final alpha=0
    // 23 Nov if ratioWinConRad > 0 and (ATot > 0 or VAir > 0) and sum(ATransparent) > 0
  ...
  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow radHeatSol[
    nOrientations](each final alpha=0  // 23 Nov if ATot > 0 and sum(ATransparent) > 0
  ...
  BaseClasses.ThermSplitter thermSplitterSolRad(
    ...
    final nOut=nDim,
    final nIn=nOrientations  // 23 Nov if ATot > 0 and sum(ATransparent) > 0
  ...
  BaseClasses.ExteriorWall extWallRC(
    ...
    final T_start=T_start) "RC-element for exterior walls"
    annotation (                                           // 23 Nov if ATotExt > 0 "RC-element for exterior walls"
                Placement(transformation(extent={{-158,-50},{-178,-28}})));
...
/*  
  ...
  "Number of non-zero wall surface areas" annotation(Evaluate=true);
  */
/*
  parameter Integer dimension = (if ATotExt > 0 then 1 else 0)+ (if ATotWin > 0 then 1 else 0)
  annotation(Evaluate=true);
  */
  /*+ (if AInt > 0 then 1 else 0)+ (if AFloor  > 0 then 1 else 0)
  
  
 /* parameter Integer dimension =
  sum({if A > 0 then 1 else 0 for A in AArray})
  annotation(Evaluate=true);
*/
  parameter Real splitFactor[nDim, 1]=
    BaseClasses.splitFacVal(nDim, 1, AArray, fill(0, 1), fill(0, 1))
    "Share of each wall surface area that is non-zero";
  parameter Real splitFactorSolRad[nDim, nOrientations]=
    BaseClasses.splitFacVal(nDim, nOrientations, AArray, AExt, AWin)
  ...
  Modelica.Blocks.Sources.Constant hConExtWall_const(
    final k=ATotExt*hConExt  // 23 Nov if ATotExt > 0
)   "Coefficient of convective heat transfer for exterior walls"
    annotation (Placement(transformation(
    extent={{5,-5},{-5,5}},
    rotation=-90,
    origin={-104,-61})));
  Modelica.Thermal.HeatTransfer.Components.Convection convWin
    "Convective heat transfer of windows"
    annotation (                                              // 23 Nov if ATotWin > 0
                Placement(transformation(extent={{-116,30},{-96,50}})));
  Modelica.Blocks.Sources.Constant hConWin_const(final k=ATotWin*hConWin)
    ...
    rotation=-90,
    origin={-106,68})));
  Modelica.Blocks.Math.Gain eRadSol[nOri](
    final k(each unit="m2")=gWin*(1 - ratioWinConRad)*ATransparent,
    u(each final unit="W/m2"),
    y(each final unit="W"   // 23 Nov if sum(ATransparent) > 0
  ...
  Modelica.Blocks.Math.Gain eConvSol[nOri](
    final k(each unit="m2")=gWin*ratioWinConRad*ATransparent,
    u(each final unit="W/m2"),
    y(each final unit="W"   // 23 Nov if ratioWinConRad > 0 and sum(ATransparent) > 0
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resExtWallWin(
    final G=min(ATotExt, ATotWin)*hRad  // 23 Nov if ATotExt > 0 and ATotWin > 0
  ...
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor senTAir "Indoor air temperature sensor"
    annotation (
    // 23 Nov if ATot > 0 or VAir > 0 "Indoor air temperature sensor"
                Placement(transformation(extent={{80,-10},{100,10}})));
  Modelica.Thermal.HeatTransfer.Sensors.TemperatureSensor senTRad "Mean indoor radiation temperatur sensor"
    annotation (
    // 23 Nov if ATot > 0 "Mean indoor radiation temperatur sensor"
                Placement(
    transformation(
    extent={{-10,-10},{10,10}},
    rotation=90,
  ...
  Modelica.Blocks.Math.Sum sumSolRad(final nin=nOrientations
    // 23 Nov if ratioWinConRad > 0 and sum(ATransparent) > 0
  ...
  Modelica.Blocks.Math.Gain mWat_flow(
    ...
    u(final unit="W"),
    y(final unit="kg/s")) "Mean indoor radiation temperatur sensor"
    annotation (
    // 23 Nov if use_moisture_balance 23 Nov and ATot >0 "Mean indoor radiation temperatur sensor"
                Placement(transformation(extent={{-200,-100},{-180,-80}})));

  Modelica.Thermal.HeatTransfer.Sources.PrescribedHeatFlow conQLat_flow
    "Converter for latent heat flow rate"
    annotation (
    // 23 Nov if use_moisture_balance and ATot >0
      Placement(transformation(extent={{-202,-130},{-182,-110}})));
...
equation
  ...
  connect(extWallRC.port_a, convExtWall.solid)
    ...
    color={191,0,0},
    smooth=Smooth.None));
  // 23 Nov  if ATotExt > 0 and ATotWin > 0 then
