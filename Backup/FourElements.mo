...
model FourElements "Thermal Zone with four elements for exterior walls,
  ...
  // 18 Nov
/*  extends ThreeElements(
   final AArray = {ATotExt, ATotWin, AInt, AFloor, ARoof});
  // extends ThreeElements(AArray={ATotExt,ATotWin,AInt,AFloor,ARoof});
  // 18 Nov - added 
  parameter Modelica.Units.SI.Area AArray[5]
    "List of all wall surface areas";
*/

  // 24 Nov
  parameter Integer idxFloorGains =
  if (not (ATotExt > 0) and not (ATotWin > 0) and not (AInt > 0) and AFloor > 0) then 1 else
  if (AFloor > 0 and (
        (ATotExt > 0 and not (ATotWin > 0) and not (AInt > 0)) or
        (not (ATotExt > 0) and ATotWin > 0 and not (AInt > 0)) or
        (not (ATotExt > 0) and not (ATotWin > 0) and AInt > 0))) then 2 else
  if (AFloor > 0 and (
        (not (ATotExt > 0) and ATotWin > 0 and AInt > 0) or
        (ATotExt > 0 and not (ATotWin > 0) and AInt > 0) or
        (ATotExt > 0 and ATotWin > 0 and not (AInt > 0)))) then 3 else 4;
  ...
  parameter Integer nRoof(min = 1) "Number of RC-elements of roof"
    annotation(Dialog(group="Roof"));
  parameter Modelica.Units.SI.ThermalResistance RRoof[nExt](each min=Modelica.Constants.small)
  ...
  parameter Modelica.Units.SI.ThermalResistance RRoofRem(min=Modelica.Constants.small)
    ...
    annotation (Dialog(group="Roof"));
  parameter Modelica.Units.SI.HeatCapacity CRoof[nExt](each min=Modelica.Constants.small)
...
protected
  // 18 Nov - added
  // Local copies to guarantee fixed-parameter evaluation for conditionals
  parameter Modelica.Units.SI.Area ATotExt_local = ATotExt;
  parameter Modelica.Units.SI.Area ATotWin_local = ATotWin;
  parameter Modelica.Units.SI.Area ATot_local    = ATot;

  // 23 Nov - converted alphaRad to hRad
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resIntRoof(
    final G=min(AInt, ARoof)*hRad) if
      ...
      rotation=-90,
        origin={186,10})));
  // 24 Nov
  parameter Boolean hasRoofWin = (ARoof > 0 and ATotWin_local > 0) annotation(Evaluate=true);

  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resRoofWin if hasRoofWin;


/*
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resRoofWin(
    final G=min(ARoof, ATotWin_local)*hRad) if 
       ARoof > 0 and ATotWin_local > 0 "Resistor between roof and windows"
      annotation (Placement(transformation(
      extent={{-10,-10},{10,10}},
  origin={-154,100})));
  */
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resExtWallRoof(
    final G=min(ATotExt_local, ARoof)*hRad) if ATotExt_local > 0 and ARoof > 0
...
/*  
  ...
  */
  ...
  // 24 Nov
  parameter Integer dimension =  (if ATotExt > 0 then 1 else 0)+ (if ATotWin > 0 then 1 else 0)+ (if AInt > 0 then 1 else 0)+ (if AFloor  > 0 then 1 else 0)+ (if ARoof  > 0 then 1 else 0)
...
equation
  ...
  // 18 Nov - added
  if not ATotExt_local > 0 and not ATotWin_local > 0 and not AInt > 0 and not AFloor > 0
    and ARoof > 0 then
    connect(thermSplitterIntGains.portOut[1], roofRC.port_a);
    connect(roofRC.port_a, thermSplitterSolRad.portOut[1]);
  elseif ATotExt_local > 0 and not ATotWin_local > 0 and not AInt > 0 and not AFloor > 0
    and ARoof > 0
     or not ATotExt_local > 0 and ATotWin_local > 0 and not AInt > 0 and not AFloor > 0
     and ARoof > 0
     or not ATotExt_local > 0 and not ATotWin_local > 0 and AInt > 0 and not AFloor > 0
     and ARoof > 0
     or not ATotExt_local > 0 and not ATotWin_local > 0 and not AInt > 0 and AFloor > 0
     and ARoof > 0 then
    connect(thermSplitterIntGains.portOut[2], roofRC.port_a);
    connect(roofRC.port_a, thermSplitterSolRad.portOut[2]);
  elseif ATotExt_local > 0 and ATotWin_local > 0 and not AInt > 0 and not AFloor > 0 and ARoof > 0
     or ATotExt_local > 0 and not ATotWin_local > 0 and AInt > 0 and not AFloor > 0 and ARoof > 0
     or ATotExt_local > 0 and not ATotWin_local > 0 and not AInt > 0 and AFloor > 0 and ARoof > 0
     or not ATotExt_local > 0 and ATotWin_local > 0 and AInt > 0 and not AFloor > 0 and ARoof > 0
     or not ATotExt_local > 0 and ATotWin_local > 0 and not AInt > 0 and AFloor > 0 and ARoof > 0
     or not ATotExt_local > 0 and not ATotWin_local > 0 and AInt > 0 and AFloor > 0
     and ARoof > 0 then
    connect(thermSplitterIntGains.portOut[3], roofRC.port_a);
    connect(roofRC.port_a, thermSplitterSolRad.portOut[3]);
  elseif not ATotExt_local > 0 and ATotWin_local > 0 and AInt > 0 and AFloor > 0 and ARoof > 0
     or ATotExt_local > 0 and not ATotWin_local > 0 and AInt > 0 and AFloor > 0 and ARoof > 0
     or ATotExt_local > 0 and ATotWin_local > 0 and not AInt > 0 and AFloor > 0 and ARoof > 0
     or ATotExt_local > 0 and ATotWin_local > 0 and AInt > 0 and not AFloor > 0 and ARoof > 0 then
    connect(thermSplitterIntGains.portOut[4], roofRC.port_a);
    connect(roofRC.port_a, thermSplitterSolRad.portOut[4]);
  elseif ATotExt_local > 0 and
    ATotWin_local > 0 and AInt > 0 and AFloor > 0 and ARoof > 0 then
...
/*    
  ...
  elseif not ATotExt > 0 and ATotWin > 0 and AInt > 0 and AFloor > 0 and ARoof > 0
    ...
    connect(thermSplitterIntGains.portOut[4], roofRC.port_a);
    connect(roofRC.port_a, thermSplitterSolRad.portOut[4]);
  elseif ATotExt > 0 and 
    ATotWin > 0 and AInt > 0 and AFloor > 0 and ARoof > 0 then
    */
  ...
  <p align=\"center\">
  <img src=\"modelica://IBPSA/Resources/Images/ThermalZones/ReducedOrder/RC/FourElements.png\" alt=\"image\"/>
  </p>
  </html>"));
end FourElements;
