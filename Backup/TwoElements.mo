...
model TwoElements
  ...
  // 18 Nov
  extends OneElement(
   final AArray = {ATotExt, ATotWin, AInt});
...
//   extends OneElement(AArray={ATotExt,ATotWin,AInt});
  ...
  BaseClasses.InteriorWall intWallRC(
    ...
    final RInt=RInt,
    final CInt=CInt,
    final T_start=T_start) "RC-element for interior walls"
    annotation (                                           // 23 Nov if AInt > 0 "RC-element for interior walls"
                Placement(transformation(extent={{182,-50},{202,-28}})));
...
protected
  Modelica.Thermal.HeatTransfer.Components.Convection convIntWall
    ...
    annotation (                                                 // 23 Nov if AInt > 0
                Placement(transformation(extent={{148,-30},{128,-50}})));
  Modelica.Blocks.Sources.Constant alphaIntWall(k=AInt*alphaInt // 23 Nov if AInt > 0
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resExtWallIntWall(
    final G=min(ATotExt, AInt)*hRad // 23 Nov if  ATotExt > 0 and AInt > 0
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor resIntWallWin(
    final G=min(ATotWin, AInt)*hRad // 23 Nov if  ATotWin > 0 and AInt > 0
)   "Resistor between interior walls and windows"
    annotation (Placement(transformation(extent={{74,-118},{94,-98}})));

  // 24 Nov

  parameter Integer dimension =  (if ATotExt > 0 then 1 else 0)+ (if ATotWin > 0 then 1 else 0)+ (if AInt    > 0 then 1 else 0)
...
equation
 ...
 // 23 Nov if not ATotExt > 0 and not ATotWin > 0 and AInt > 0 then
  ...
  <p align=\"center\">
  <img src=\"modelica://IBPSA/Resources/Images/ThermalZones/ReducedOrder/RC/TwoElements.png\" alt=\"image\"/>
  </p>
  </html>"),
