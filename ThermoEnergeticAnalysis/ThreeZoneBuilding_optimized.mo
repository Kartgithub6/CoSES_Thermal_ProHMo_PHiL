...
model ThreeZoneBuilding_optimized
  ...
  // ⭐⭐⭐ OPTIMIZED VALVE MINIMUMS - Key to preventing overheating!
  parameter Real yMin_roof=0.03 " 18 Jan Reduced to 3% Roof valve minimum 8%";
  parameter Real yMin_living=0.03 "Living valve minimum 3% (priority zone)";
  parameter Real yMin_cellar=0.02 "Reduced from 5% to 2% Cellar valve minimum 5% (REDUCED from 20%!)";
  ...
  CoSES_Thermal_ProHMo_PHiL.HydronicSystem.SystemWithZoneAndHydronics_optimized roof_hydSys(
    redeclare package Medium=Medium,
    Q_flow_nominal=2500,  // Roof: 2500W (adequate)
    T_a_nominal=343.15,
    T_b_nominal=323.15,
    TAir_nominal=293.15,
    fraRad=0.35,
  ...
  BuildingSystem.Building.HeatedZone roof_zone(
    ...
    annotation(Placement(visible=true, transformation(origin={70,35}, extent={{-20,-20},{20,20}}, rotation=0)));
  ...
  Modelica.Blocks.Sources.Constant TRefConst_living(k=TRef_living)
    ...
    annotation(Placement(visible=true, transformation(origin={-160,30}, extent={{-10,-10},{10,10}}, rotation=0)));

  Modelica.Blocks.Sources.Constant TRefConst_roof(k=TRef_roof)
    "Roof temperature setpoint"
    annotation(Placement(visible=true, transformation(origin={-160,55}, extent={{-10,-10},{10,10}}, rotation=0)));
...
equation
  ...
  connect(port_a, qvMedium.port_a)
    annotation(Line(points={{-200,0},{-180,0}}, color={0,127,255}, thickness=0.5));
  connect(qvMedium.port_b, TMedium.port_a)
    annotation(Line(points={{-160,0},{-150,0}}, color={0,127,255}, thickness=0.5));
  connect(TMedium.port_b, teeMain.port_1)
    annotation(Line(points={{-130,0},{-120,0}}, color={0,127,255}, thickness=0.5));

  // ⭐ LIVING PRIORITY: Direct connection from main tee
  connect(teeMain.port_2, valve_living.port_a)
    annotation(Line(points={{-100,0},{-100,-20},{-70,-20},{-70,0},{-60,0}},   color={0,127,255}, thickness=0.5));
  ...
  connect(teeMain.port_3, teeCellarRoof.port_1)
    annotation(Line(points={{-110,10},{-110,75},{-80,75}}, color={0,127,255}));
  ...
  connect(TReturn.port_b, port_b)
    annotation(Line(points={{170,-100},{-200,-100}}, color={0,127,255}, thickness=0.5));
  ...
  connect(living_hydSys.port_zone, living_zone.heatPort)
    annotation(Line(points={{0,8},{-26,8},{-26,-4},{-28,-4},{-28,-38},{30,-38},{
          30,8},{38,8},{38,18.4},{48.8,18.4}},                      color={191,0,0}));
  connect(roof_hydSys.port_zone, roof_zone.heatPort)
    annotation(Line(points={{0,54},{0,60},{40,60},{40,43.4},{48.8,43.4}},
                                                                    color={191,0,0}));
  ...
  connect(TRefConst_living.y, PI_living.u_s)
    annotation(Line(points={{-149,30},{-102,30}}, color={0,0,127}));
  connect(living_zone.TZone, PI_living.u_m)
    annotation(Line(points={{62,-10.4},{110,-10.4},{110,5},{-90,5},{-90,18}},
                                                                        color={0,0,127}));
  connect(PI_living.y, valve_living.opening)
    annotation(Line(points={{-79,30},{-70,30},{-70,15},{-50,15},{-50,8}}, color={0,0,127}));
  connect(TRefConst_living.y, living_zone.TZoneRef)
    annotation(Line(points={{-149,30},{-140,30},{-140,50},{100,50},{100,18},{89.6,
          18}},                                                                       color={0,0,127}));

  // Roof control loop
  connect(TRefConst_roof.y, PI_roof.u_s)
    annotation(Line(points={{-149,55},{-102,55}}, color={0,0,127}));
  connect(roof_zone.TZone, PI_roof.u_m)
    annotation(Line(points={{62,14.6},{110,14.6},{110,30},{-90,30},{-90,43}},
                                                                          color={0,0,127}));
  ...
  connect(TRefConst_roof.y, roof_zone.TZoneRef)
    annotation(Line(points={{-149,55},{-140,55},{-140,70},{100,70},{100,43},{89.6,
          43}},                                                                       color={0,0,127}));
  ...
  connect(WindowShadingConst_living.y, living_zone.WindowShading)
    annotation(Line(points={{61,30},{49.4,30},{49.4,-9.8}},
                                                      color={0,0,127}));
  connect(WindowShadingConst_roof.y, roof_zone.WindowShading)
    annotation(Line(points={{61,55},{49.4,55},{49.4,15.2}},
                                                      color={0,0,127}));
  ...
  connect(Q_radiation_living.y, living_zone.Q_radiation_W)
    annotation(Line(points={{41,40},{50,40},{50,16},{26,16}}, color={0,0,127}));
  connect(Q_radiation_roof.y, roof_zone.Q_radiation_W)
    annotation(Line(points={{41,65},{50,65},{50,41},{26,41}}, color={0,0,127}));
  ...
  connect(nPersons_living, living_zone.nPersons)
    annotation(Line(points={{-220,140},{-210,140},{-210,130},{30,130},{30,22},{103.2,
          22}},                                                                         color={0,0,127}));
  connect(nPersons_roof, roof_zone.nPersons)
    annotation(Line(points={{-220,100},{-210,100},{-210,110},{25,110},{25,47},{103.2,
          47}},                                                                         color={0,0,127}));
  ...
  connect(P_appliances_living_W, living_zone.P_appliances_W)
    annotation(Line(points={{230,140},{220,140},{220,24},{26,24}}, color={0,0,127}));
  connect(P_appliances_roof_W, roof_zone.P_appliances_W)
    annotation(Line(points={{230,100},{220,100},{220,49},{26,49}}, color={0,0,127}));
  ...
  connect(living_zone.TZone, TZone_living)
    annotation(Line(points={{62,-10.4},{210,-10.4},{210,40}},
                                                        color={0,0,127}));
  connect(roof_zone.TZone, TZone_roof)
    annotation(Line(points={{62,14.6},{210,14.6},{210,-60}},
                                                         color={0,0,127}));
  ...
  connect(PI_living.y, valve_living_opening)
    annotation(Line(points={{-79,30},{-70,30},{-70,50},{190,50},{190,0},{210,0}}, color={0,0,127}));
...
<p><b>⭐⭐⭐ PROFESSIONAL HVAC DESIGN with OPTIMIZED TOPOLOGY</b></p>
...
<pre>
Supply → teeMain
           ├─ port_2 → valve_living → living ✅ PRIORITY!
           └─ port_3 → teeCellarRoof
                          ├─ port_2 → valve_cellar → cellar
                          └─ port_3 → valve_roof → roof
...
<li><b>Cellar: 2500W</b> - ⭐ REDUCED from 4000W (was 60% oversized!)</li>
...
<li><b>Cellar: 5%</b> (was 20%) - ⭐ 75% REDUCTION!</li>
<li><b>Roof: 8%</b> (was 10%) - Moderate</li>
</ul>

<h5>WHY THIS WORKS:</h5>

<p><b>Cellar Heat Balance at 5% minimum (-2°C outdoor):</b></p>
<pre>
Heat Loss:
...
Controller has FULL RANGE (5%-100%) to modulate ✅
Temperature: Stable at 18-19°C ✅
</pre>
